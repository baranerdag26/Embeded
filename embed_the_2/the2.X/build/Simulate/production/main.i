...
__attribute__((picinterrupt(("high_priority")))) void HandleInterrupt()
{
...
void submit_piece()
{
    ...
    {
        ports = parse_and_assign(active_piece_record);
    }
    else
    {
        ports = parse_and_assign(active_piece);
    }

    LATC_value = PORTC | LATC_old | ports[0];
    LATD_value = PORTD | LATD_old | ports[1];
    LATE_value = PORTE | LATE_old | ports[2];
    LATF_value = PORTF | LATF_old | ports[3];
...
void shift_down_piece(uint32_t *active_piece)
{
    ...
    uint8_t byte0 = (uint8_t)(ports[0] << 1);
    uint8_t byte1 = (uint8_t)(ports[1] << 1);
    uint8_t byte2 = (uint8_t)(ports[2] << 1);
    uint8_t byte3 = (uint8_t)(ports[3] << 1);
...
void main()
{
    ...
    _delay((unsigned long)((1000)*((40UL * (1000UL * 1000UL))/4000.0)));
